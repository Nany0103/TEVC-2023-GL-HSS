function [Subset,selInd] = LGIHSSEmbed(PopObj,selNum,ref)
    a = size(PopObj, 1);
    M = size(PopObj, 2);
    selectedPop = [];
    selInd = [];
    if a<=selNum
       Subset=PopObj; 
        ...
        for n = 1 : a
           ...
           heap.hvc(n) = HVC(currentPop,ref,1);
        ...
        while size(selectedPop,1)<selNum
            ...
            while true
                ...
                [heap.hvc(1),num] = HVC(currentPop,ref,size(currentPop,1));
                numOfSol = numOfSol+num;
...
function [Score,num] = HVC(Population,RefPoint,index)
    data = Population;
    data1 = data;
    s = data1(index,:);
    data1(index,:)=[];
    data1 = max(s,data1); 
    num = sum(NDSort(data1,1)==1);
