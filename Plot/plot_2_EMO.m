...
for objInd = 1:2
    ...
    for proInd = 1:6
        ...
        for runInd = 1:runNum
            ...
            HV(runInd,2,rowInd) = hvGLNAGO_IDSS{runInd}(end); RT(runInd,2,rowInd) = rtGLNAGO_IDSS{runInd}(end);
            HV(runInd,3,rowInd) = hvGLNAGO_DSS{runInd}(end); RT(runInd,3,rowInd) = rtGLNAGO_DSS{runInd}(end);
            %% Plot Figure
...
function write_table(HV,RT,pro,obj)
    ...
    FID = fopen('./HV_Initialization_Comparison_EMO.tex','w');
    for rowInd = 1:size(HV,3)
        if mod(rowInd-1,2)==0
    ...
    fclose(FID);
    FID = fopen('./RT_Initialization_Comparison_EMO.tex','w');
    for rowInd = 1:size(HV,3)
        if mod(rowInd-1,2)==0
            fprintf(FID,'\\multirow{2}{*}{%s}',pro{floor((rowInd-1)/2)+1});
        else
