...
for objInd = 1:2
    ...
    for proInd = 1:6
        ...
        for runInd = 1:runNum
            ...
            j = 100;
            % load GA100+DL4
            [hv,rt] = load_DL4(ref,M,pro{proInd},selNum,alpha,runInd);
            hv_GA100(runInd) = hv(1); rt_GA100(runInd) = rt(1);
            % load TGA+DL4: Proposed one
            [hv,rt] = load_DL4_TGA(ref,M,pro{proInd},selNum,alpha,numSol2,runInd);
            hv_TGA(runInd) = hv(1); rt_TGA(runInd) = rt(1);
            hvDL4_TGA{runInd} = hv; rtDL4_TGA{runInd} = rt;
            % load LGI-HSS
            [hvLGI(runInd),rtLGI(runInd)] = load_LGI(ref,M,pro{proInd},selNum,runInd);
            % load TGA+GLNAGO
            [hv,rt] = load_GLNAGO_TGA(ref,M,pro{proInd},selNum,numSol2,runInd);
            ...
            saveas(f,sprintf('./Figure/Overall/M%d_%s_%d.png',M,pro{proInd},runInd));
            close all;
        end
    end
end
...
function[hv,rt] = load_DL4_TGA(ref,M,proDispName,selNum,alpha,numSol2,runInd)
    ...
    fileName = sprintf('./Result/Result New/DLHSS4_TGA_M%d_%s_selNum=%d_alpha=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,alpha,numSol2,runInd);
    dataSelSet = load(fileName).dataSelSet;
    rt = load(fileName).runTime;
    rt = rtGA+rt;
    fileName = sprintf('./Result/Result New/HV_DLHSS4_TGA_M%d_%s_selNum=%d_alpha=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,alpha,numSol2,runInd);
...
function[hv,rt] = load_GLNAGP_TGA(ref,M,proDispName,selNum,numSol2,runInd)
    fileName = sprintf('./Result/Result New/TGAHSS_M%d_%s_selNum=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,numSol2,runInd);
    rtGA = load(fileName).runTime;
...
function[hv,rt] = load_GLNAGPTest_TGA(ref,M,proDispName,selNum,numSol2,runInd)
    fileName = sprintf('./Result/Result New/TGAHSS_M%d_%s_selNum=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,numSol2,runInd);
    rtGA = load(fileName).runTime;
    fileName = sprintf('./Result/Result New/GLHSSNAGPTest_TGA_M%d_%s_selNum=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,numSol2,runInd);
    dataSelSet = load(fileName).dataSelSet;
    rt = load(fileName).runTime;
    rt = rtGA+rt;
    fileName = sprintf('./Result/Result New/HV_GLHSSNAGPTest_TGA_M%d_%s_selNum=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,numSol2,runInd);
    hv = zeros(1,length(rt));
    for i=1:length(rt)
        hv(i) = stk_dominatedhv(dataSelSet(:,:,i),ref);
...
function[hv,rt] = load_GLNAGO_TGA(ref,M,proDispName,selNum,numSol2,runInd)
    fileName = sprintf('./Result/Result New/TGAHSS_M%d_%s_selNum=%d_numSol2=%d_runInd=%d.mat', ...
        M,proDispName,selNum,numSol2,runInd);
    rtGA = load(fileName).runTime;
...
function[hv,rt] = load_DL4(ref,M,proDispName,selNum,alpha,runInd)
    j = 100;
    fileName = sprintf('./Result/Result New/GAHSS%d_M%d_%s_selNum=%d_runInd=%d.mat', ...
        j,M,proDispName,selNum,runInd);
    rtGA = load(fileName).runTime;
    fileName = sprintf('./Result/Result New/DLHSS4_GA%d_M%d_%s_selNum=%d_alpha=%d_runInd=%d.mat', ...
        j,M,proDispName,selNum,alpha,runInd);
    dataSelSet = load(fileName).dataSelSet;
    rt = load(fileName).runTime;
    rt = rtGA+rt;
    fileName = sprintf('./Result/Result New/HV_DLHSS4_GA%d_M%d_%s_selNum=%d_alpha=%d_runInd=%d.mat', ...
        j,M,proDispName,selNum,alpha,runInd);
    if exist(fileName,'file')
        hv = load(fileName).hv;
    else
...
function[hv,rt] = load_LGI(ref,M,proDispName,selNum,runInd)
